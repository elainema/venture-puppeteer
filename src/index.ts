...
import { getPageHeight,checkImagePath } from "./utils"
...
const generatePng: (config: IRequestConfig) => Promise<IRequestRes | unknown> = (config) => { 
  return new Promise((resolve, reject) => {
    const _genarate = async () => {
      ...
      checkImagePath(name, "images").then(async (imagesPath) => { 
        const images = [];
        // 生成每个部分的图片文件
